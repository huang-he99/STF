...
class ResidulBlock(nn.Module):
    def __init__(self, in_channels, out_channels, sampling=None):
        ...
        self.residual = nn.Sequential(*residual)
        self.transform = (
            transform[0] if len(transform) == 1 else nn.Sequential(*transform)
        )
...
class ResidulBlockWithSpectralNorm(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.residual = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True),
            SpectralNorm2d(Conv3X3NoPadding(in_channels, in_channels, stride=2)),
            nn.BatchNorm2d(in_channels),
            nn.LeakyReLU(inplace=True),
            SpectralNorm2d(nn.Conv2d(in_channels, out_channels, 1)),
        )
        self.transform = SpectralNorm2d(
