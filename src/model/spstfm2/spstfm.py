...
from src.model.spstfm2.ksvd import KSVD
...
class SPSTFM(object):
    ...
    def training_dictionary_pair(
        ...
        r"""
        Args:
            coarse_img_01: (c,h,w) numpy.ndarray
            coarse_img_03: (c,h,w) numpy.ndarray
            fine_img_01: (c,h,w) numpy.ndarray
            fine_img_03: (c,h,w) numpy.ndarray
        """

        channel_num = coarse_img_01.shape[0]
        ...
        with mp.Pool(processes=channel_num) as pool:
            dictionary_pair_list = pool.starmap(
                ...
                (np.split(img_samples, channel_num, axis=0)),
            )
        ...
        coarse_dictionary = np.stack(coarse_dictionary_list, axis=0)
        fine_dictionary = np.stack(fine_dictionary_list, axis=0)
        sparse_matrix = np.stack(sparse_matrix_list, axis=0)
    ...
    def img_preprocessing_train_phase(
        ...
        Args:
            ...
            fine_img_03: (c,h,w) numpy.ndarray
        Output:
            img_samples: (c,HW,sample_num) numpy.ndarray
        """
        coarse_diff_31 = coarse_img_03 - coarse_img_01  # (c,h,w) numpy.ndarray
        fine_diff_31 = fine_img_03 - fine_img_01  # (c,h,w) numpy.ndarray
        (
            coarse_diff_31_mean,
            coarse_diff_31_std,
    ...
    def get_img_statistic(self, img):
        r"""
        Args:
            img: (c,h,w) numpy.ndarray
        Output:

            img_mean: (c,1,1) numpy.ndarray
            img_std: (c,1,1) numpy.ndarray
        """
        img_mean = np.mean(img, axis=(1, 2), keepdims=True)
        img_std = np.std(img, axis=(1, 2), keepdims=True)
        return img_mean, img_std

    def get_patch_vectors(self, img_array):
        r"""
        Args:
            img_array: (c,h,w) numpy.ndarray
        Output:
            img_patch_vectors: (c,HW,L) numpy.ndarray
        """
    ...
    def reconstruction(
        ...
        Args:
            coarse_img_01: (c,h,w) numpy.ndarray
            coarse_img_02: (c,h,w) numpy.ndarray
            coarse_img_03: (c,h,w) numpy.ndarray
            fine_img_01: (c,h,w) numpy.ndarray
            fine_img_03: (c,h,w) numpy.ndarray
            coarse_dictionary: (c,L,atom_num) numpy.ndarray
            fine_dictionary: (c,L,atom_num) numpy.ndarray
        Output:
            fine_img_02: (c,h,w) numpy.ndarray
        """
        channel_num = coarse_img_01.shape[0]
        ...
        with mp.Pool(processes=channel_num) as pool:
            fine_diff_img_patch_vectors_list = pool.starmap(
                ...
                (
                    zip(
                        np.split(coarse_img_patch_vectors_21, channel_num, axis=0),
                        np.split(coarse_img_patch_vectors_32, channel_num, axis=0),
                        np.split(coarse_dictionary, channel_num, axis=0),
                        np.split(fine_dictionary, channel_num, axis=0),
                    )
                ),
            )
        ...
        fine_diff_img_patch_vectors_21 = np.stack(
            fine_diff_img_patch_vectors_21_list, axis=0
        )  # (c,pHpW,L)
        fine_diff_img_patch_vectors_32 = np.stack(
    ...
    def img_preprocessing_inference_phase(
        ...
        Args:
            ...
            fine_img_03: (c,h,w) numpy.ndarray
        Output:
            coarse_img_patch_vectors_21: (c,HW,L) numpy.ndarray
            coarse_img_patch_vectors_32: (c,HW,L) numpy.ndarray
            fine_img_patch_vectors_01: (c,HW,L) numpy.ndarray
            fine_img_patch_vectors_03: (c,HW,L) numpy.ndarray
        """
        coarse_diff_21 = coarse_img_02 - coarse_img_01  # (c,h,w) numpy.ndarray
        coarse_diff_32 = coarse_img_03 - coarse_img_02  # (c,h,w) numpy.ndarray
        (
            coarse_diff_21_mean,
            coarse_diff_21_std,
    ...
    def HRDI_reconstruction(
        ...
        r"""
        Args:
            coarse_img_patch_vectors_21: (HW,L) numpy.ndarray
            coarse_img_patch_vectors_32: (HW,L) numpy.ndarray
            fine_img_patch_vectors_01: (HW,L) numpy.ndarray
            fine_img_patch_vectors_03: (HW,L) numpy.ndarray
            coarse_dictionary: (atom_dim,atom_num) numpy.ndarray
            fine_dictionary: (atom_dim,atom_num) numpy.ndarray
        Output:
            reconstructed_fine_img: (HW,L) numpy.ndarray
        """
        coarse_img_patch_vectors_21 = coarse_img_patch_vectors_21[0]
        coarse_img_patch_vectors_32 = coarse_img_patch_vectors_32[0]
        coarse_dictionary = coarse_dictionary[0]
        fine_dictionary = fine_dictionary[0]

        sparse_matrix_21 = linear_model.orthogonal_mp(
            coarse_dictionary,
            coarse_img_patch_vectors_21,
            n_nonzero_coefs=self.sparisty,
        )
        sparse_matrix_32 = linear_model.orthogonal_mp(
            coarse_dictionary,
            coarse_img_patch_vectors_32,
            n_nonzero_coefs=self.sparisty,
        )
    ...
    def get_temporal_weight(self, coarse_img_01, coarse_img_03, coarse_img_02):
        ...
        dBU_21 = np.abs(BU_02 - BU_01)
        dBU_32 = np.abs(BU_03 - BU_02)

        dBU_21_patch_vectors = self.get_patch_vectors(dBU_21)  # (1,pHpW,L)
        dBU_32_patch_vectors = self.get_patch_vectors(dBU_32)

        dBU_21_patch_vectors = (
            np.mean(dBU_21_patch_vectors, axis=1, keepdims=True) + EPSILON
        )  # (1,1,L)
        dBU_32_patch_vectors = (
            np.mean(dBU_32_patch_vectors, axis=1, keepdims=True) + EPSILON
        )

        dBU_31 = BU_03 - BU_01
        dBU_31_patch_vectors = self.get_patch_vectors(dBU_31)
        dBU_31_patch_vectors = np.mean(dBU_31_patch_vectors, axis=1, keepdims=True)
    ...
    def patch_to_img(self, patch_vectors, out_size):
        ...
        patch_vectors = torch.from_numpy(patch_vectors).float()
...
if __name__ == '__main__':
    ...
    coarse_img_01 = coarse_img_01.transpose(2, 0, 1)
    coarse_img_02 = coarse_img_02.transpose(2, 0, 1)
    coarse_img_03 = coarse_img_03.transpose(2, 0, 1)
    fine_img_01 = fine_img_01.transpose(2, 0, 1)
    fine_img_02 = fine_img_02.transpose(2, 0, 1)
    fine_img_03 = fine_img_03.transpose(2, 0, 1)
