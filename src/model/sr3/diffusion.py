...
class GaussianDiffusion(nn.Module):
    ...
    def set_new_noise_schedule(self, schedule_opt, device="cuda"):
    ...
    @torch.no_grad()
    def p_sample_loop(self, coarse_img):
        # device = self.betas.device
        # sample_inter = 1 | (self.num_timesteps // 10)
        img = torch.randn_like(coarse_img)
        for i in tqdm(
            reversed(range(0, self.num_timesteps)),
            desc="sampling loop time step",
            total=self.num_timesteps,
        ):
            img = self.p_sample(img, i, condition_x=coarse_img)
        return img

    @torch.no_grad()
    def sample(self, coarse_img):
        return self.p_sample_loop(coarse_img)
